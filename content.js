...
// --- DOWNLOAD DOCS: open preview → click Download → close → next ---
  if (msg.action === 'clickDocs') {
    ...
    // Filter by extension (language-agnostic filename read)
    ...
    (async () => {
      ...
      for (const btn of buttons) {
        ...
        // 2) poll up to ~12s for the Download button (slow-internet safe)
        let dl = null;
        for (let i = 0; i < 40; i++) {
          await wait(300);
        ...
        if (dl) {
          ...
          await wait(1800); // let download start
        }

        // 4) close preview (X button, Escape as fallback)
        let x = null;
        for (let i = 0; i < 20; i++) {
        ...
        await wait(1500); // buffer before next doc
